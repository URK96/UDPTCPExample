...
typedef void char;  // Type used for raw data on this platform
#endif
...
const unsigned int BUFSIZE = 32;  // Size of receive buffer
...
SocketException::SocketException(const string& message, bool inclSysMsg) throw() : userMessage(message) {
    ...
    if (inclSysMsg) {
        userMessage.append(": ");
        strerror_s(buffer, BUFSIZE, errno);
...
static void fillAddr(const string& address, unsigned short port, sockaddr_in& addr) {
    ...
    addr.sin_port = htons(port);  // Assign port in network char order
}
...
unsigned short Socket::resolveService(const string& service, const string& protocol) {
    ...
    if ((serv = getservbyname(service.c_str(), protocol.c_str())) == NULL)
        ...
        return ntohs(serv->s_port); /* Found port (network char order) by name */
}
...
void CommunicatingSocket::send(const void* buffer, int bufferLen) throw(SocketException) {
    if (::send(sockDesc, (char*)buffer, bufferLen, 0) < 0) {
...
int CommunicatingSocket::recv(void* buffer, int bufferLen) throw(SocketException) {
    int rtn;
    if ((rtn = ::recv(sockDesc, (char*)buffer, bufferLen, 0)) < 0) {
...
TCPSocket::TCPSocket(const string& foreignAddress, unsigned short foreignPort) throw(SocketException) : CommunicatingSocket(SOCK_STREAM, IPPROTO_TCP) { connect(foreignAddress, foreignPort); }
...
UDPSocket::UDPSocket() throw(SocketException) : CommunicatingSocket(SOCK_DGRAM, IPPROTO_UDP) { setBroadcast(); }
...
void UDPSocket::sendTo(const void* buffer, int bufferLen, const string& foreignAddress, unsigned short foreignPort) throw(SocketException) {
    ...
    if (sendto(sockDesc, (char*)buffer, bufferLen, 0, (sockaddr*)&destAddr, sizeof(destAddr)) != bufferLen) {
...
void UDPSocket::setMulticastTTL(unsigned char multicastTTL) throw(SocketException) {
    if (setsockopt(sockDesc, IPPROTO_IP, IP_MULTICAST_TTL, (char*)&multicastTTL, sizeof(multicastTTL)) < 0) {
...
void UDPSocket::joinGroup(const string& multicastGroup) throw(SocketException) {
    ...
    if (setsockopt(sockDesc, IPPROTO_IP, IP_ADD_MEMBERSHIP, (char*)&multicastRequest, sizeof(multicastRequest)) < 0) {
...
void UDPSocket::leaveGroup(const string& multicastGroup) throw(SocketException) {
    ...
    multicastRequest.imr_multiaddr.s_addr = inet_addr(multicastGroup.c_str());
    multicastRequest.imr_interface.s_addr = htonl(INADDR_ANY);
    if (setsockopt(sockDesc, IPPROTO_IP, IP_DROP_MEMBERSHIP, (char*)&multicastRequest, sizeof(multicastRequest)) < 0) {
